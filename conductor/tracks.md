...
## Build status (verified 2026-06-26; amended 2026-07-03 — 09 shipped, 35/37 archived, 08 in progress)
...
| **Shipped (training-signal hardening)** | 37 training-signal-hardening *(code + tests, 2026-07-02; **delivers 35 nudge** as Phase D)* | The push from "safely not-worse" to "measurably better, fast, steerable". **Phase 0** dataset contract v1.1 (`outcome`/`judge_score`/`judge_verdict`/`tier`/`chosen_over`, additive per-variant, byte-identical v1.0 round-trip, ledger-hash-invariant). **Phase A** outcome-stamped ingest (`tool_use`→`tool_result` correlation, retry-collapse to one success w/ `chosen_over`, bare-failure exclusion → `rejected.jsonl` sidecar, verified-success 8000-char cap, `[ingest].tier`). **Phase B** `LlmPairJudge` + `[judge]` (min_score/on_error keep\|drop/batch/sample_k) wired post-mine/pre-enqueue (txn untouched) + standalone `evolve dataset judge` + manifest `eval_report` rollup (judged_fraction/judge_mean_score/outcome_verified_fraction) + `tier` field (most-restrictive-wins) — the lexame marked-expertise fields. **Phase C** planner routes `skill`/`reasoning_edit` (no silent `completion`→Prose degrade) + `[domain]` parameterization (byte-identical scrt default) + `rejection_sample` (best-of-N) + `evolve dataset expand` (Evol/Self-Instruct). **Phase D (= track 35)** `evolve ambient nudge` → atomic `nudge.json`, daemon consume-once at step boundary, safe-live allowlist + reject-with-reason, `kind:"nudge"`/`kind:"steering_compliance"` evolution-log rows. **Phase E** `[daemon].objective` default `end_task` + Python live-calib sourcing + same-model rotary-kwargs fix. All Rust tested ML-free (`cargo test` + `--features train` + clippy green); Python syntax-verified (pytest blocked on a pre-existing torch/transformers env incompatibility, not track code). | 35, 32, 31, 29, 26, 15, 9 |
| **Shipped (modalities)** | 09 modalities *(code + tests, commit `fb192c9` 2026-06-28; archived)* | Skill-ingestion + reasoning-edit generation: `GenExample::Skill`/`ReasoningEdit` variants threaded through generate → dataset → probe/score → branch/train + export, `tests/modalities.rs` coverage (209-test sweep green at ship). Status records lagged the code (metadata/plan/this table said unbuilt — fixed 2026-07-03); implementation diverged from plan naming (`Skill`, serde `skill`). Track 37 phase C added the planner ROUTING for both modalities. |
|| **Roadmap — live (unbuilt but product-aligned)** | 08 extract-publish *(BLOCKED on crates.io)* · 36 install-ux *(IN PROGRESS 2026-07-04)* · 39 native-inference · 40 delegation-contract | The only unbuilt tracks kept as roadmap. **08** (swap the scrt-core git dep → published crate + cut a release): pre-publish work landed 2026-07-03 (CHANGELOG, README feature-flag matrix, CI 3-leg workflow, green feature sweep, API-surface doc + staged one-line swap, in-tree-crate retirement decision) — the remaining verify→flip→tag steps stay externally blocked on scrt-core hitting crates.io. **36** install scripts created (`scripts/install.sh`, `scripts/install.ps1`); remaining: CI release workflow, `evolve setup` CLI subcommand, README quickstart. *(Track **35 nudge** was **delivered by track 37 Phase D** — see the training-signal-hardening row; deeper trainer science — DPO training, loss curricula — is candidate **track 38**.)* **39 native-inference** + **40 delegation-contract** (2026-07-03) are the newest roadmap entries: native-only candle serving (arbitrary per-shard placement, no-GGUF prompting, per-arch seam fundamentals) and the evolve⇄lexame capability-card/daisy-chain contract. *(An adaptive-compute/early-exit exploration was oracle-measured and purged the same day — no depth redundancy in Granite-4-h-tiny, ceiling ≤1.1×; that research continues outside this repo.)* |
...
1. **Live ambient GPU run (26)** — the *only* genuine completion item. The
   ...
   not code.
2. **Publish + release (08)** — **IN PROGRESS 2026-07-03**: all pre-publish work
   landed (CHANGELOG, README feature-flag matrix, CI workflow, green feature
   sweep, API-surface doc + staged one-line swap, in-tree-crate retirement
   decision). Remaining verify→flip→tag steps blocked on scrt-core being
   published to crates.io (an external dependency).
3. ~~**New modalities (09)**~~ — **SHIPPED 2026-06-28** (commit `fb192c9`):
   skill-ingestion + reasoning-edit rows through the full pipeline; archived
   under `_archived/completed/`.
...
| :- | :--- | :--- | :--- | :--- |
| **00** | [repo-skeleton-config](tracks/_archived/completed/00-repo-skeleton-config/) | 1 | Workspace, `EvolveConfig` (toml load+validate), work-dir layout, **PyO3 feature stub**. No ML. | — |
| **01** | [discover](tracks/_archived/completed/01-discover/) | 2 | `discover.rs` over scrt-core (search + palace + simhash dedup/cluster) → `DiscoveredContext` → `discovered.json`. No ML. | 00 |
| **02** | [generate-api-backend](tracks/_archived/completed/02-generate-api-backend/) | 3 | `GenBackend` trait, `ApiEndpoint` impl, prompt templates, `Dataset` JSONL writer/reader, **dataset→Python export over the PyO3 bridge**. End-to-end discover→dataset, no local model. | 01 |
| **03** | [generate-local-candle](tracks/_archived/completed/03-generate-local-candle/) | 4 | `LocalCandle` GenBackend (candle inference) behind the same trait + `train` feature. | 02 |
| **04** | [train-lora](tracks/_archived/completed/04-train-lora/) | 5 | `TrainingPreset` trait, model loader (`model.rs` per-arch seam, ONE arch first), LoRA injection + training loop → `adapter.safetensors`. **PyO3 training-step seam** so `peft`/`trl` can drive it. | 02 (data), 03 (model loader shared) |
| **19** | [python-train-infer](tracks/_archived/completed/19-python-train-infer/) | — (core validation) | Standalone Python `scrt_evolve_train` (transformers LoRA) + `scrt_evolve_infer` (base+adapter A/B), driven from the Rust CLI via subprocess. The **PRIMARY real-model training/inference path** (candle = fixture). dataset.jsonl is the contract. | 02 (dataset), 03/04 (candle fixture it validates) |
| **20** | [learning-by-doing](tracks/_archived/completed/20-learning-by-doing/) | — (product capstone) | **Incremental multi-goal LEARNING-BY-DOING evolution**: `[[goals]]` (name/topic/tag) in `evolve.toml`; a paired **`scrt-evolve` SKILL** steers a frontier agent to stash goal-tagged findings as it works → the palace + transcripts become the curriculum. Per-goal eval-gated rounds (discover→generate→train→eval→keep\|rollback via track 15), generation-improves-itself (track 11 flywheel), and a bounded **scheduler** across goals. Orchestration over shipped tracks — no new ML. Makes the DESIGN daemon buildable + safe. | 01 (palace-search), 02, 19 (train), 10 (eval), 15 (txn), 11 (regen) |
...
| **09** | [modalities-skill-reasoning](tracks/_archived/completed/09-modalities-skill-reasoning/) | — (new) | **SHIPPED 2026-06-28** (commit `fb192c9`). New generation modalities: **skill ingestion** (`Skill` rows — absorb a SKILL.md into callable behavior) + **reasoning-step modification** (`ReasoningEdit` rows — insert/correct/prune/reorder CoT). Flows through the existing planner→generate→dataset→export pipeline. | 02 (gen/dataset) |
| **10** | [eval-harness](tracks/_archived/completed/10-eval-harness/) | — (lane foundation) | **Shared** `ProbeSet` + `Scorer` (`ScoreReport`) + `StepVerdict` + the executable `gate.rs`. Scoring backends: `api` (no ML), **`pyo3`→`transformers`** (perplexity/exit-depth), `candle` (optional). Built FIRST so 11/12/15 stop assuming an evaluator nobody built. | 02 (`ApiEndpoint`), 00 (`pyo3`) |
...
| **15** | [self-regulation](tracks/_archived/completed/15-self-regulation/) | — (capstone) | **Homeostasis**: transactional evolution (checkpoint → evaluate via **track 10** → keep\|rollback); **self-pruning** (auto expert eviction/merge native-Rust + gated base sparsity via **PyO3→torch**); catastrophe → **auto-rollback + quarantine by `gen`-provenance + halt**. Makes the daemon safe. | 10 (eval), 11, 12, 13, 14 |
...
| **35** | [nudge-live-retuning](tracks/_archived/completed/35-nudge-live-retuning/) | **DELIVERED via 37 Phase D (2026-07-02) — archived** | **Nudge**: live-retune a RUNNING ambient daemon (goal weights, active focus, throttle knobs) without a restart — a `nudge.json` control-file the loop polls + consumes at the step boundary (mirrors the `daemon.run` stop-file idiom), validated against a safe-live allowlist; restart-required knobs (model_path, fractional shape, rotation_blocks) rejected-with-reason. `teach` stays the *content* knob; nudge is the *direction* knob. Surfaced as a `kind:"nudge"` evolution-log row + in `watch status`. Start from `PASSOFF.md`. | 26, 31, 15 |
| **37** | [training-signal-hardening](tracks/_archived/completed/37-training-signal-hardening/) | **SHIPPED (code + tests, 2026-07-02) — archived** | **Training-signal hardening** — push the signal from "safely not-worse" to "measurably better, fast, and steerable" (2026-07-02 review findings): **(0)** dataset contract v1.1 (additive `outcome`/`judge_score`/`judge_verdict`/`tier`/`chosen_over`); **(A)** outcome-stamped ingest — correlate `tool_use`→`tool_result`, exclude bare failures, **retry-collapse** to (chosen, rejected) preference pairs; **(B)** per-pair **LLM data judge** (`LlmPairJudge`, `ChatTransport`-injected like tracks 31/32; `[judge].min_score` + configurable fail-open/closed) with judged/outcome stats rolled into the branch manifest `eval_report` + `tier` sovereignty (most-restrictive-wins) — the lexame **marked-expertise** trust fields; **(C)** high-throughput synthesis — rejection sampling (`candidates_per_seed`, judge-ranked keep-k), Evol/Self-Instruct `dataset expand`, planner routes `skill`/`reasoning_edit`, `[domain]` parameterization de-hardcodes scrt (byte-identical default); **(D)** steerable loop — **absorbs track 35 nudge** + extends its allowlist with data-layer knobs + steering-compliance in `watch trend`; **(E)** surgical training fixes (daemon default `end_task`, live-queue calib batches, rotary kwargs on same-model shard path). Research basis: AlpaGasus 2307.08701, Deita 2312.15685, RAFT 2304.06767, Self-Instruct 2212.10560, Evol-Instruct 2304.12244, Magpie 2406.08464, DPO 2305.18290 (contract only). Track-15 txn untouched; ML-free acceptance. | 35 (absorbs), 32, 31, 29, 26, 15, 09 |
| **39** | [native-inference](tracks/39-native-inference/) | **PLANNED** (spec; no impl) | **Native inference engine (candle) — the ONLY serving path, model-generic.** Own the forward pass: prompt an evolved model (base + `adapter.safetensors` directly, or the exported GGUF via candle's quantized loader) **without the merge→GGUF→llama.cpp chokepoint**; `[serve.placement]` with **arbitrary interleaved `gpu_shards` layer→device arrays** (llama.cpp could only express a contiguous `n_gpu_layers` prefix); in-process adapter hot-reload at keep-commit (upgrades track 33's swap); per-arch **`seam_points()`** exposing the model's structural joints (e.g. Granite's Mamba→attention boundary, the `bench/seam_distill` seam) for in-process instrumentation/capture — the seam fundamentals. **Native-only (2026-07-03 decision): the llama.cpp sidecar is RETIRED** per-family once Phase-0 parity gates pass — GGUF stays an *export/interop* format, not a serving dependency; Granite-4-h's Mamba2/SSD ops are **implemented in-runtime (Phase B)** rather than falling back. **Model-generic** via an `ArchAdapter` layer-enumeration seam; **branch-from-any-base invariant**: whatever the branch factory can train, native serve loads — or `branch create` refuses at preflight (trainable ⇒ servable, checked up front). Inference-only (candle `train` stays fixture). | 19, 27, 29 |
| **40** | [delegation-contract](tracks/40-delegation-contract/) | **PLANNED** (spec; contract v0.1 drafted) | **Cross-repo delegation contract (evolve ⇄ lexame)** — versions the **capability card** (the inbound routing surface: an abstract, *measured* representation of what a model is optimized/capable of reasoning on), the shareable **delegation shard** (card + provenance), and **daisy-chain** semantics; canonical **`DELEGATION-CONTRACT.md` mirrored in BOTH repos' `.omc/`** (scrt-evolve + laxame-hivemind — sibling of `SCRT-EVOLVE-INTEGRATION.md`). **Evolve owns**: card schema + emission — manifest **`competence_profile`** (per-domain measured confidence backed by probe-set hashes; extends `router_signature`→`eval_report`/`tier` lineage; **non-gameable-by-construction**: claims derive from judged probe runs, domains from the probe taxonomy only, cards stamp the checkpoint ordinal + refuse-stale), `evolve branch card` (measured-data export), probe-pack export, conformance fixtures. **Lexame owns**: **the card/shard artifact itself** (assembly, publication, versioning as a network object — 2026-07-03 decision), transport/discovery (NAT hole-punch + WebRTC), challenge-response verification (§4.5), hop budget/TTL + loop prevention, per-hop provenance, trust weighting, **end-to-end confidence = product of hop confidences** (forwarding a raw upstream score is a violation), delegation-policy training against the live network. An outbound trigger scalar is contract-reserved but **deferred** (no implementation in this track). | 29, 37 |
