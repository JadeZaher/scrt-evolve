...
| CLI `evolve train generate [--in discovered.json] [--backend api]` → `dataset.jsonl`; runs standalone from disk | `crates/scrt-evolve-cli/src/main.rs` lines 228–238: `cmd_generate()` loads discovered context, calls `scrt_evolve::generate::run()`, writes to workdir. CLI test (no discover call) integration verified by structure. |
